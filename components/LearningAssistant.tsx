...
type GenerationProgress = { label: string; startedAt: number; timedOut: boolean };
...
const LearningAssistant: React.FC<LearningAssistantProps> = ({ currentUser, view, onViewChange, onOpenClassroom, onOpenTask }) => {
  ...
  const generationDialogRef = useRef<HTMLDivElement>(null);
  ...
  const isGenerating = creating || textbookCreating;
  const pageLocked = Boolean(generationProgress && !generationProgress.timedOut);
  const beginGeneration = (label: string): boolean => {
    if (generationInFlightRef.current) return false;
    generationInFlightRef.current = true;
    setGenerationProgress({ label, startedAt: Date.now(), timedOut: false });
    return true;
  };
  ...
  const elapsedLabel = `${Math.floor(generationElapsedSeconds / 60)}分${String(generationElapsedSeconds % 60).padStart(2, '0')}秒`;
  ...
  useEffect(() => {
    ...
    const updateElapsed = () => {
      ...
      if (elapsed >= 300) setGenerationProgress(current => current ? { ...current, timedOut: true } : null);
    };
    updateElapsed();
  ...
  useEffect(() => {
    if (generationProgress && !generationProgress.timedOut) generationDialogRef.current?.focus();
  }, [generationProgress?.startedAt, generationProgress?.timedOut]);
  ...
  const createWrongReview = async () => {
    if (!selectedItems.length || isGenerating || !beginGeneration('错题讲解与测验')) return;
    ...
    finally { setCreating(false); finishGeneration(); }
  };
  ...
  const createChapterTask = async () => {
    if (!selectedAction || !selectedBook || !selectedChapter || !actionDetail?.available || isGenerating) return;
    const chapterIds = selectedAction === 'assessment' ? assessmentChapterIds : [selectedChapter.id];
    if (!chapterIds.length) return;
    if (!beginGeneration(ACTION_COPY[selectedAction].label)) return;
    ...
    finally { setTextbookCreating(false); finishGeneration(); }
  };

  const createOlympiadTask = async () => {
    if (!olympiadBookId || isGenerating || !beginGeneration('奥数模拟考试')) return;
    setTextbookCreating(true); setTextbookError(''); setCreatedTitle(''); setCreatedTaskId('');
    try {
  ...
  return <section className="mx-auto max-w-6xl space-y-6 animate-fade-in" aria-labelledby="assistant-title">
    ...
    <div role="tablist" aria-label="学习方式" className="flex w-fit max-w-full items-center gap-1 overflow-x-auto rounded-2xl border border-cyber-border/60 bg-cyber-surface/50 p-1 backdrop-blur-md">
      {SOURCE_TABS.map(tab => { const active = tab.id === 'wrong' ? view === 'wrong' : view === 'textbook' && textbookMode === (tab.id === 'olympiad' ? 'olympiad' : 'chapter'); const Icon = tab.icon; return <button key={tab.id} type="button" role="tab" aria-selected={active} disabled={pageLocked} onClick={() => selectSource(tab.id)} className={`inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border px-4 py-2 text-sm font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-neon-blue disabled:cursor-not-allowed disabled:opacity-50 ${active ? 'border-neon-blue/40 bg-gradient-to-r from-neon-blue/25 to-neon-purple/20 text-neon-blue shadow-glow-sm' : 'border-transparent text-cyber-muted hover:bg-white/5 hover:text-cyber-text'}`}><Icon size={16} />{tab.label}</button>; })}
    </div>

...
const ClassroomTaskHub: React.FC<ClassroomTaskHubProps> = ({ currentUser, books, subPath = '', onOpenLegacy, onOpenHub }) => {
  ...
  const load = async (cursor?: string, append = false) => {
    ...
    try {
      const page = await fetchClassroomTasks({ ownerId: currentUser.id, subject: subject || undefined, type: type || undefined, status: status || undefined, bookId: bookId || undefined, cursor, limit: 20 });
  ...
  useEffect(() => { if (!taskId) void load(); }, [currentUser.id, subject, type, status, bookId, taskId]);
  useEffect(() => {

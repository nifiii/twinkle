...
const TASK_CATEGORIES = [
  { id: 'course', label: '课程学习', types: ['courseware', 'classroom_quiz'] },
  { id: 'wrong', label: '错题讲解', types: ['wrong_review'] },
  { id: 'assessment', label: '模拟考试', types: ['assessment'] },
  { id: 'other', label: '其他', types: ['english_listening', 'math_thinking', 'quiz_result', 'video'] },
] as const;
type TaskCategory = typeof TASK_CATEGORIES[number]['id'];
...
const ClassroomTaskHub: React.FC<ClassroomTaskHubProps> = ({ currentUser, books, subPath = '', onOpenLegacy, onOpenHub }) => {
  ...
  const [category, setCategory] = useState<TaskCategory>('course');
  ...
  const primaryAction = (item: ClassroomTaskSummary) => {
    ...
    if (item.generationStatus !== 'ready') {
      ...
      if (item.blockedReason) return <span className="max-w-sm text-sm text-amber-300">{item.blockedReason}</span>;
      return item.source === 'task' && item.retryable
  ...
  return <section className="mx-auto max-w-6xl space-y-6 animate-fade-in" aria-labelledby="classroom-title">
    ...
    <div role="tablist" aria-label="学习内容分类" className="grid max-w-full grid-cols-2 gap-1 rounded-2xl border border-cyber-border/60 bg-cyber-surface/50 p-1 backdrop-blur-md sm:grid-cols-4">{TASK_CATEGORIES.map(item => <button key={item.id} type="button" role="tab" aria-selected={category === item.id} onClick={() => setCategory(item.id)} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-neon-blue ${category === item.id ? 'border-neon-blue/40 bg-gradient-to-r from-neon-blue/25 to-neon-purple/20 text-neon-blue shadow-glow-sm' : 'border-transparent text-cyber-muted hover:bg-white/5 hover:text-cyber-text'}`}><span>{item.label}</span><span className="text-xs opacity-75">{categoryCounts.get(item.id) || 0}</span></button>)}</div>
    ...
    {loading ? <div role="status" className="flex min-h-48 items-center justify-center gap-2 rounded-2xl border border-cyber-border/60 bg-cyber-surface/50 text-cyber-muted"><Loader2 className="animate-spin" />正在读取{subject}待学习任务</div> : !visibleItems.length ? <div role="status" className="rounded-2xl border border-cyber-border/60 bg-cyber-surface/50 p-10 text-center text-sm text-cyber-muted">{subject}暂时没有{selectedCategory.label}任务。可前往学习小助手创建内容。</div> : <section aria-labelledby="selected-category-title" className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 id="selected-category-title" className="text-base font-semibold text-cyber-text">{selectedCategory.label}</h2><span className="rounded-lg bg-white/5 px-2 py-1 text-sm text-cyber-muted">{visibleItems.length} 项待学习</span></div><div className="space-y-3">{visibleItems.map(item => <article key={item.id} className="grid gap-3 rounded-xl border border-cyber-border/60 bg-cyber-surface/60 p-4 shadow-glow-sm md:grid-cols-[minmax(0,1fr)_auto]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-neon-blue/10 px-2 py-1 text-xs font-medium text-neon-blue">{TYPE_LABEL[item.taskType] || item.taskType}</span>{item.source === 'legacy' && <span className="text-xs text-cyber-muted">历史内容</span>}</div><h3 className="mt-2 break-words text-base font-semibold text-cyber-text">{item.title}</h3><p className="mt-1 break-words text-sm text-cyber-muted">{item.book?.title || '独立学习内容'}{item.chapterTitles.length ? ` · ${item.chapterTitles.join('、')}` : ''}</p></div><div className="flex items-center md:justify-end">{primaryAction(item)}</div></article>)}</div></section>}

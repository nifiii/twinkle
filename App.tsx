  ...
  const renderContent = () => {
    try {
      switch (activeTab) {
        ...
        case 'assistant':
          return <LearningAssistant currentUser={currentUser} view={assistantSubPath === 'wrong' ? 'wrong' : 'textbook'} onViewChange={(view) => handleTabChange('assistant', view)} onOpenClassroom={() => handleTabChange('tutor')} onOpenTask={(taskId) => handleTabChange('tutor', `task/${encodeURIComponent(taskId)}`)} />;
        case 'decommissioned':

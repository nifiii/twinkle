...
课件生成变为可排队、可恢复、可重试的任务，完成后可直接打开并保持现有保存行为；模型和持久化耗时可单独定位。
...
- 课件 Job handler、结果校验/保存适配、前端状态接入、固定输入基准夹具。

Non-goals:

- 不在本任务中减少既有章节、正文或讲稿内容预算。
- 不改变课件 JSON schema、错题关联或测验功能。

Acceptance:

- 生成、保存、刷新后打开课件均与现有行为一致。
- JSON 格式异常和上游限流只标记相应任务阶段并支持受控重试。
- 记录模型首响应、完整响应、JSON 校验、保存和页面可用时间。
- 在 T-001 固定输入三并发 5 次测试中记录最慢值；若超过 60 秒，输出证据并停止在内容范围决策点，不静默降质。

Freedom:

- 可提取 prompt 构造、JSON 校验和持久化为内部 helper。
- 不得修改用户可见内容长度或将半成品标为完整课件。

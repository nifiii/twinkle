...
学生可以进入 `#learn`，选择家庭共享或自己已归档且元数据完整的教材与章节，并在三次点击内看到该学科已支持的听力、视频、练习或测试入口；元数据缺失时被清晰阻止。英语显示听力、视频和测试；数学显示视频、思维练习和测试；科学显示视频和测试；其他学科仅显示测试。
...
- 不得改变路由契约、行动名称、教材锚点校验要求或 UI/UE 文档中的可访问性要求；不得展示服务端学科契约明确不支持的行动。

...
当前仅限内网家庭使用，WebSocket 暂从查询参数读取 `ownerId`，后端必须验证该用户存在并限制每个用户同时仅一个会话。该识别方式不是认证，不能用于公网、端口暴露或多租户部署；迁移到公网前必须改为服务端会话或签名令牌，且禁止信任浏览器提供的用户标识。
...
5. 若发现内网边界被破坏或 `ownerId` 可被未授权设备调用，立即关闭 `LIVE_TUTOR_DOUBAO_ENABLED`，保持其他业务可用，待真实认证接入后再开启。

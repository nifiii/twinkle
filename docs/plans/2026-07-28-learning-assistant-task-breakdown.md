...
- 只接受当前家庭学生档案下、同一选定学科的 1-10 个有效题目引用。引用可来自错题本 `scanned_item` 或课堂作答 `quiz_result`；误识别题可由调用方排除而不改源数据。
- 成功后恰有一个 `wrong_review` 任务、一个讲解链接和一个测验链接；只有 `scanned_item` 来源保持兼容链接记录，`quiz_result` 来源仅记录在任务快照。
...
- 数学奥数模拟考试通过 `source.options.examMode=olympiad` 与 `olympiadBookId` 指定资料；仅在上传资料的学科、奥数标记及标注年级与当前学生/教材范围匹配时可用。该资料只提供标题、分类和标签风格摘要，绝不提供正文、原题或答案；常规教材模拟考试不受该条件阻断。
...
- 模拟考试只提交具有稳定目录 ID 的具体章节；目录单元标题缺少 ID 时，选择单元内任一章节后创建模拟考试会自动聚合该单元全部有效章节，且不得提交空章节数组。

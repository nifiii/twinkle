...
- 仅对 `scanned_items.type='wrong_problem'` 的精确历史前缀 `/opt/hl-os/data/`，将其相对路径映射到当前数据卷作存在性检查；映射后不存在的文件列入 `delete.files.missing`，不执行文件暂存但仍可随其错题记录在 apply 中删除。该例外不适用于教材、上传试卷或其他扫描资料，也不放行任何其他数据卷外路径。

...
import { createReviewItems } from './answerReviewService.js';
...
type StoredAttempt = { id: string; paperId: string; ownerId: string; answersJson: string; reviewSnapshotJson: string | null; status: AttemptStatus; submittedAt: number | null; createdAt: number; updatedAt: number };
...
function paperQuestions(database: Database.Database, paperId: string, ownerId: string) {
...
function questionIds(database: Database.Database, paperId: string, ownerId: string) {
  return new Set(paperQuestions(database, paperId, ownerId).map(question => question.id).filter((id): id is string => Boolean(id)));
}
...
function output(row: StoredAttempt) {
  return {
    id: row.id, paperId: row.paperId, ownerId: row.ownerId, status: row.status,
...
export function updatePaperAttempt(id: unknown, request: Record<string, unknown>, database: Database.Database = db) {
  const ownerId = parseLearningOwnerId(request.ownerId); const attemptId = requiredString(id, 'id');
  const row = database.prepare('SELECT * FROM paper_attempts WHERE id = ? AND ownerId = ?').get(attemptId, ownerId) as StoredAttempt | undefined;
  ...
  const snapshot = submit ? JSON.stringify(createReviewItems(paperQuestions(database, row.paperId, ownerId), answers)) : row.reviewSnapshotJson;
  database.prepare('UPDATE paper_attempts SET answersJson = ?, reviewSnapshotJson = ?, status = ?, submittedAt = ?, updatedAt = ? WHERE id = ?').run(JSON.stringify(answers), snapshot, submit ? 'submitted' : 'draft', submit ? now : null, now, attemptId);
